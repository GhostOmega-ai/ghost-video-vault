...
- Restored pinch zoom while browser fullscreen is active
- Removed the fullscreen CSS rule that forcibly cancelled video transforms
- Kept `object-fit: contain` so the complete video still fits before zooming
- Fullscreen and orientation refitting now reapplies the user's current zoom
- Gesture surface now explicitly fills the entire fullscreen viewport
